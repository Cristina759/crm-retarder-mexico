...
const ESTADO_CONFIG: Record<CotEstado, { label: string; color: string }> = {
    ...
    negociacion: { label: 'Negociacin', color: 'bg-orange-100 text-orange-700' },
...
const INITIAL_COTIZACIONES: DemoCotizacion[] = [
    ...
    { id: '2', numero: 'COT-00088', empresa: 'Logstica Global', vendedor: 'Ana G.', subtotal: 81896.55, iva: 13103.45, total: 95000, estado: 'aceptada', vigencia_dias: 30, fecha: '2026-02-12', items: 3, orden_id: '5', orden_numero: 'OS-00047' },
    ...
    { id: '6', numero: 'COT-00084', empresa: 'Fletes del Pacfico', vendedor: 'Pedro V.', subtotal: 103448.28, iva: 16551.72, total: 120000, estado: 'borrador', vigencia_dias: 30, fecha: '2026-02-08', items: 6 },
...
export default function CotizacionesPage() {
    ...
    const handleDelete = useCallback((id: string) => {
        // Envolvemos el confirm en un pequeo delay para que la interaccin del click se registre primero
        // y no bloquee el "Interaction to Next Paint" (INP)
        setTimeout(async () => {
            if (!await confirmModal('Ests seguro de que deseas eliminar esta cotizacin? Esto tambin eliminar cualquier Orden de Servicio vinculada. Esta accin no se puede deshacer.')) return;
            ...
            startTransition(async () => {
                try {
                    ...
                    // 2. Ahora eliminamos la cotizacin
                    ...
                    toast.error(`Error al eliminar la cotizacin: ${error.message || 'Error desconocido'}`);
                } finally {
    ...
    const handleCreateOrden = async (cot: any) => {
        setIsProcessing(true);
        try {
            // 1. Generar nmero de orden secuencial basado en el total existente en Supabase
            // Fallback determinstico con timestamp si la query falla (evita colisiones)
            ...
            // 3. Actualizar la cotizacin con la referencia a la orden
            const { error: updateError } = await supabase
                .from('cotizaciones')
                .update({
                    orden_id: insertedData[0].id,
                    // Si el esquema tiene orden_numero en la tabla cotizaciones, lo actualizamos tambin
                    // Por si acaso, lo manejamos dinmicamente o confiamos en el join futuro
                })
                .eq('id', cot.id);

            // Nota: Si el esquema de cotizaciones NO tiene orden_id, esto fallar silenciosamente o dar error.
            // He verificado que S tiene orden_id en pasos anteriores.
                    ...
                    <table className="w-full text-sm">
                        <thead className="bg-retarder-gray-50">
                            <tr className="border-b border-retarder-gray-200">
                                <th className="text-left py-3 px-2 sm:px-4 text-[10px] font-semibold text-retarder-gray-400 uppercase">Cotizacin</th>
                                ...
                                <th className="text-left py-3 px-2 sm:px-4 text-[10px] font-semibold text-retarder-gray-400 uppercase hidden md:table-cell">Vnculo</th>
                                <th className="text-left py-3 px-2 sm:px-4 text-[10px] font-semibold text-retarder-gray-400 uppercase hidden sm:table-cell">Accin</th>
                            </tr>
                        </thead>
                        <tbody>
                            {loading ? (
