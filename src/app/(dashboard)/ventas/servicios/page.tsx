...
export default function CotizadorServiciosPage() {
    ...
    const handleCreateCot = async () => {
        ...
        setIsCreating(true);
        try {
            const clienteMatch = clientes.find(c => c.id === newCot.empresa_id) || CLIENTES_REALES.find(c => c.id === newCot.empresa_id);
    ...
    return (
        <div className="space-y-6 pb-12">
                                        ...
                                        <div className="bg-white rounded-2xl border border-retarder-gray-200 overflow-hidden shadow-sm">
                                            ...
                                            {expandChecklist && (
                                                <div className="px-5 py-4 space-y-4">
                                                    {SERVICE_TYPES[0].checklist.map(group => (
                                                        <div key={group.title}>
                                        ...
                                        <div className="flex justify-between">
                                            ...
                                            <div className="text-right space-y-1">
                                                <div className="grid grid-cols-[auto_1fr] gap-x-3 text-left">
                                                    ...
                                                    <span className="font-bold">SERVICIO:</span> <span className="uppercase">{activeService.label}</span>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="space-y-1">
                                        <div className="space-y-1">
                                            ...
                                            {traslado > 0 && <PriceLine label={`Desplazamiento / Traslado (${cantidad} u.)`} mxn={traslado * cantidad} />}
                                        </div>
                                        <div className="py-2" />
            ...
            <AnimatePresence>
                {showForm && (
                    <>
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowForm(false)} className="fixed inset-0 bg-retarder-black/60 backdrop-blur-md z-[80]" />
                        <motion.div
                            initial={{ opacity: 0, scale: 0.95, y: 30 }}
                            animate={{ opacity: 1, scale: 1, y: 0 }}
                            exit={{ opacity: 0, scale: 0.95, y: 30 }}
                            className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-[2rem] shadow-2xl z-[90] overflow-hidden"
                        >
                            <div className="p-8">
