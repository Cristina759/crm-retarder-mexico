...
export function OrdenDetailPanel({ orden, onClose, onUpdate }: OrdenDetailPanelProps) {
    ...
    const fetchSurvey = async () => {
        ...
        try {
            ...
            // Tabla encuestas puede no existir a˙n ‚Äî no bloquear el panel
        }
    ...
    const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>, tipo: 'documento' | 'foto_antes' | 'foto_despues') => {
        ...
        try {
            // Validar UUID antes de cualquier operaciÛn de DB
            const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(orden.id);
            ...
            if (isUUID) {
                const { error: regError } = await StorageService.registerEvidence({
                    ...
                    tipo: tipo as any,
                    archivo_url: url,
                    descripcion: tipo === 'documento' ? 'Orden de Compra' : tipo === 'foto_antes' ? 'Evidencia Antes' : 'Evidencia DespuÈs',
                });
                ...
                const demoEvidencia: Evidencia = {
                    ...
                    tipo: tipo as any,
                    archivo_url: url,
                    descripcion: tipo === 'documento' ? 'Orden de Compra (Demo)' : tipo === 'foto_antes' ? 'Evidencia Antes (Demo)' : 'Evidencia DespuÈs (Demo)',
                    subido_por: 'demo',
    ...
    const handleUpdateTecnico = async (newTecnico: string) => {
        ...
        try {
            ...
            setError(`Error: ${err.message || 'No se pudo asignar el tÈcnico.'}`);
        } finally {
    ...
    const handleAdvanceState = async () => {
        ...
        try {
            ...
            // Estas Ûrdenes ya completaron su proceso real y no necesitan evidencias digitales
            ...
            // Solo aplicar validaciones estrictas a Ûrdenes que est·n en fases tempranas
            if (!isAlreadyAdvanced) {
                // 2. Para avanzar a fase operativa: requiere tÈcnico + N¬∫ OS fÌsica + foto
                ...
                if (operativeStates.includes(nextState)) {
                    if (!localTecnico) {
                        throw new Error('Se requiere asignar un tÈcnico antes de avanzar.');
                    }
                    if (!numeroOrdenFisica.trim()) {
                        throw new Error('Se requiere capturar el N¬∫ de Orden de Servicio FÌsica (papel) antes de avanzar.');
                    }
                    const hasFotoOrden = evidencias.some(e => e.tipo === 'foto_antes');
                    if (!hasFotoOrden) {
                        throw new Error('Se requiere subir la foto de la Orden de Servicio FÌsica (papel) antes de avanzar.');
                    }
                }
                ...
                if (nextState === 'evidencia_cargada') {
                    ...
                    if (photosCount < 2) {
                        throw new Error('Se requiere cargar fotos de "Antes" y "DespuÈs" para avanzar.');
                    }
                }
            }
            ...
            if (nextState === 'pagado' && !isAlreadyAdvanced) {
                const hasFotoOrden = evidencias.some(e => e.tipo === 'foto_antes');
                if (!hasFotoOrden) {
                    throw new Error('No se puede pasar a Pagado sin subir las FOTOS DE LA ORDEN fÌsica.');
                }
            ...
            if (isUUID) {
                ...
                if (existing) {
                    // La orden ya existe en Supabase ‚Äî solo actualizar estado
                    ...
                    // La orden viene de DEMO_ORDENES y no existe en Supabase ‚Äî insertarla
                ...
                // Si es demo, simulamos el Èxito para el flujo de la UI
            }
            ...
            setError(err.message || 'OcurriÛ un error al avanzar el estado.');
        } finally {
    ...
    const handleDeleteOrden = async () => {
        ...
        // Si no est· en modo confirmaciÛn, activarlo
        if (!showDeleteConfirm) {
            setShowDeleteConfirm(true);
            // Auto-cancelar despuÈs de 5 segundos
                ...
                <>
                    ...
                    >
                        ...
                        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="space-y-1">
                                    <div className="flex items-center gap-1.5 text-retarder-gray-400">
                                        <User size={14} />
                                        <span className="text-[10px] font-semibold uppercase tracking-wide">TÈcnico</span>
                                    </div>
                                    {(orden.estado === 'asignacion_tecnico' || !localTecnico || orden.estado === 'cotizacion_aceptada') ? (
                                        <div className="relative pl-5">
                                            ...
                                            >
                                                <option value="">‚ö†Ô∏è Seleccionar tÈcnico...</option>
                                                {TECNICOS.map(t => (
                            ...
                            {/* Orden de Servicio FÌsica (TÈcnico) */}
                            <div>
                                <h3 className="text-[10px] font-semibold uppercase tracking-wider text-retarder-gray-400 mb-2">
                                    <ClipboardList size={12} className="inline mr-1" />
                                    Orden de Servicio FÌsica
                                </h3>
                                <div className="bg-amber-50 border border-amber-200/50 rounded-xl p-4 space-y-3">
                                    <p className="text-[10px] text-amber-700 font-medium">
                                        El tÈcnico captura el n˙mero de la orden impresa y sube la foto del documento.
                                    </p>

                                    {/* N˙mero de orden fÌsica */}
                                    <div>
                                        <label className="text-[10px] font-bold uppercase tracking-wider text-amber-800 mb-1 block">N¬∫ Orden FÌsica (Papel)</label>
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <label className={cn(
                                            ...
                                            <span className="text-[10px] font-bold text-retarder-gray-600 uppercase">AÒadir OC</span>
                                        </label>
                                        <label className={cn(
                                            "flex items-center justify-center gap-2 p-2.5 rounded-xl border-2 border-dashed border-retarder-gray-200 hover:border-retarder-red hover:bg-retarder-red/5 transition-all cursor-pointer",
                                            isUploading && "opacity-50 cursor-not-allowed"
                                    ...
                                    {isLoadingSurvey ? (
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div className="grid grid-cols-3 gap-2">
                                                ...
                                                <div className="text-center p-2 bg-retarder-gray-50 rounded-lg">
                                                    <p className="text-[8px] text-retarder-gray-400 uppercase">TÈcnico</p>
                                        ...
                                        {evidencias.find(e => e.tipo === 'firma') ? (
                                            ...
                                            <SignaturePad
                                                ...
                                                onSave={async (blob) => {
                                                    ...
                                                    try {
                                                        const file = new File([blob], 'firma_cliente.png', { type: 'image/png' });
                                                        // La lÛgica de guardado sigue igual pero usando el blob del componente
                                                        const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(orden.id);
                                                        const bucket = 'firmas';
                                                        const fileName = `${orden.id}/firma_cliente.png`;
                            ...
                            <div className="flex gap-2">
                                ...
                                {isAdmin && (
                                    showDeleteConfirm ? (
                                        ...
                                        >
                                            <Trash2 size={16} />
                                            {isAdvancing ? 'Borrando...' : '¬øConfirmar borrado?'}
                                        </button>
                                    ) : (
                                        <button
                                            onClick={handleDeleteOrden}
