-- ============================================================
-- FIX 1: enum oportunidades (si da error de enum 'seguimiento')
-- ============================================================
DO $$ BEGIN
  BEGIN
    ALTER TYPE oportunidad_estado ADD VALUE 'seguimiento';
  EXCEPTION WHEN duplicate_object THEN NULL;
  END;
END $$;

UPDATE oportunidades SET estado = 'seguimiento_activo' WHERE estado = 'seguimiento';

-- ============================================================
-- FIX 2: Catálogos (idempotente — se puede correr N veces)
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_mo_categoria  ON mano_de_obra(categoria);
CREATE INDEX IF NOT EXISTS idx_mo_activo     ON mano_de_obra(activo);
CREATE INDEX IF NOT EXISTS idx_ref_categoria ON refacciones(categoria);
CREATE INDEX IF NOT EXISTS idx_ref_activo    ON refacciones(activo);

ALTER TABLE mano_de_obra ENABLE ROW LEVEL SECURITY;
ALTER TABLE refacciones  ENABLE ROW LEVEL SECURITY;

DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='mano_de_obra' AND policyname='service_role_all') THEN
    CREATE POLICY "service_role_all" ON mano_de_obra FOR ALL TO service_role USING (true) WITH CHECK (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='refacciones' AND policyname='service_role_all') THEN
    CREATE POLICY "service_role_all" ON refacciones FOR ALL TO service_role USING (true) WITH CHECK (true);
  END IF;
END $$;

-- Limpiar datos previos si hubo intento parcial
DELETE FROM mano_de_obra WHERE true;
DELETE FROM refacciones  WHERE true;

-- ── MANO DE OBRA ELÉCTRICO ──────────────────────────────────
...
-- ── REFACCIONES NEUMÁTICO ───────────────────────────────────
INSERT INTO refacciones (nombre, categoria, precio_venta) VALUES
  ('CONEXION NEUMATICA 1/4', 'NEUMÁTICO', 219.31),
  ('MANGUERA DE 1/4 POR METRO', 'NEUMÁTICO', 37.73),
  ('CONEXION NEUMATICA 3/8', 'NEUMÁTICO', 237.50),
  ('CONEXION T NEUMATICA 3/8', 'NEUMÁTICO', 271.55),
  ('MANGUERA DE 3/8 POR METRO', 'NEUMÁTICO', 66.38),
  ('REDUCCION BUSHING 3/8 A 1/4', 'NEUMÁTICO', 45.10),
  ('BULBO DE AIRE', 'NEUMÁTICO', 1044.25);

-- ── REFACCIONES TORNILLERÍA ─────────────────────────────────
INSERT INTO refacciones (nombre, categoria, precio_venta) VALUES
  ...
  ('ARANDELA GRANDE 3/4 X 2.5', 'TORNILLERÍA', 30.00),
