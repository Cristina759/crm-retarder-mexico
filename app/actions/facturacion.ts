...
export async function obtenerFacturas(): Promise<{ data: FacturaRow[]; error: string | null }> {
  try {
    const { data: rows, error } = await supabaseAdmin
      ...
      .select('id, numero, numero_os_manual, numero_factura, monto_factura, concepto_factura, fecha_vencimiento, estado_facturacion, created_at, empresa_id, cotizacion_id')
    ...
    (ncs ?? []).forEach(nc => {
     ...
     const enriched: FacturaRow[] = ((rows as any[]) ?? []).map(r => {
       ...
       let bruto = Number(r.monto_factura) || 0;
       if (bruto === 0 && cot) bruto = Number(cot.total_mxn) || 0;

        const ncMonto = ncMap.get(r.id) || 0;
        const monto_neto = bruto - ncMonto;
        ...
        if (r.estado_facturacion === 'pagada' && total_pagado === 0) {
          total_pagado = monto_neto;
        }
 
        const saldo_pendiente = Math.max(0, monto_neto - total_pagado);
 
        return {
           id: r.id,
           numero: r.numero_os_manual || r.numero,
           numero_factura: r.numero_factura,
           monto_factura: bruto, // Bruto para que el usuario vea el original
           monto_neto,         // Neto para totales
           concepto_factura: cot?.tipo || 'Servicio',
...
export async function obtenerResumenFacturacion(): Promise<{
  ...
  try {
    ...
    const { data: facts, error: errFacts } = await supabaseAdmin
      .from('ordenes_servicio')
      .select('monto_factura, estado_facturacion, cotizacion_id, abonos, numero_factura')
    ...
    // Cargar Notas de Crédito agrupadas por os_id para el cálculo preciso
    const { data: allNCs } = await supabaseAdmin.from('notas_credito').select('os_id, monto');
    const ncMap = new Map<string, number>();
    (allNCs ?? []).forEach(nc => {
      if (nc.os_id) ncMap.set(nc.os_id, (ncMap.get(nc.os_id) || 0) + (Number(nc.monto) || 0));
    });

    ((facts as any[]) ?? []).forEach(r => {
      let bruto = Number(r.monto_factura) || 0;
      if (bruto === 0 && r.cotizacion_id) {
    ...
    const totalNotasCredito = (notas ?? []).reduce((s: number, r) => s + (Number(r.monto) || 0), 0);
...
export async function registrarPago(id: string, pago: { monto: number, fecha: string, referencia: string }) {
  try {
    ...
    let montoTotal = (os as any).monto_factura || 0;
    if (montoTotal === 0 && (os as any).cotizacion_id) {
