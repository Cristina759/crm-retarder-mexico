    ...
    (ncs ?? []).forEach(nc => {
     ...
     const enriched: FacturaRow[] = (rows ?? []).map(r => {
       ...
       if (finalMonto === 0 && cot) finalMonto = Number(cot.total_mxn) || 0;
        ...
        // NOTA: El monto de la factura debe ser el bruto para evitar doble deducción en el reporte.
        // La resta de notas de crédito se hará a nivel de reporte/resumen.
        const cot = cotMap.get(r.cotizacion_id || '');
        let finalMonto = Number(r.monto_factura) || 0;
        if (finalMonto === 0 && cot) finalMonto = Number(cot.total_mxn) || 0;
       ...
       let total_pagado = abonos.reduce((s, a: any) => s + (Number(a?.monto) || 0), 0);
...
export async function obtenerResumenFacturacion(): Promise<{
  ...
  try {
    // Usamos el mismo filtro que obtenerFacturas para asegurar consistencia total
      const { data: facts, error: errFacts } = await Promise.all([
        supabaseAdmin
          .from('ordenes_servicio')
      ...
      ]).then(([f, n]) => [f, n.error ? n.error : n.data]);
