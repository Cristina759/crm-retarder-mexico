...
export async function obtenerResumenFacturacion(): Promise<{
  ...
  try {
    ...
    const totalNotasCredito = (ncs ?? []).reduce(
      (s, r) => s + (Number(r.monto) || 0), 0
    );
 
    const totalFacturado = totalFacturadoCents / 100;
    const totalCobrado = totalCobradoCents / 100;
    const totalNetoFacturado = totalFacturado;
    const totalPendiente = Math.max(0, totalFacturado - totalCobrado - totalNotasCredito);
